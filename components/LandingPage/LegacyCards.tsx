...
const LegacyCards = ({ id, active, artist }: any) => {
    ...
    useEffect(() => {
        if (active == id)
            setPos(0)
        else if ((9 + active - 1) % 9 == id)
            setPos(-1)
        else if ((9 + active + 1) % 9 == id)
            setPos(1)
        else if ((9 + active - 2) % 9 == id)
            setPos(-2)
        else if ((9 + active + 2) % 9 == id)
            setPos(2)
        else setPos(10)
    ...
    return (
        ...
        w-[100%] min-h-[400px] md:h-[600px]  lg:h-[700px] relative  flex flex-col `}>
            ...
            <div className={`h-[50%]  rounded-lg  w-[100%]  z-10   transition-all text-white flex flex-col justify-end pb-8 items-center
        ...
        ${(pos == 10) && "opacity-0 absolute -bottom-[40%]"}
           ...
           `}>
                ...
                <div className="z-10 text-5xl md:text-6xl lg:text-7xl leading-7 md:leading-none tracking-[10px]">
                    {artist.name}
                </div>
